...
# LLM-клиент создаём лениво — только для ask, не для ingest/eval
_CLIENT = None
_MODEL = None
...
def _get_splitter():
    """Ленивый импорт RecursiveCharacterTextSplitter (не тянет torch в глоб. область)."""
...
def ingest(naive: bool = False):
    ...
    chunk_fn = chunk_text_naive if naive else chunk_text
    label = "FIXED-SIZE (2000)" if naive else "RECURSIVE (512/80)"
    print(f"\nИспользую стратегию чанкинга: {label}", flush=True)
