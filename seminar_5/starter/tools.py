...
# ===========================================================================
# 3c. Сравнение метрики за два периода
# ===========================================================================


METRIC_TO_FUNC = {
    "fx_USD": lambda d: get_fx_rate("USD", d),
    "fx_EUR": lambda d: get_fx_rate("EUR", d),
    "fx_CNY": lambda d: get_fx_rate("CNY", d),
    "key_rate": lambda d: get_key_rate(d),
    "cpi": lambda d: get_inflation(int(d[:4]), int(d[5:7])),
    "unemployment": lambda d: get_unemployment(int(d[:4]), int(d[5:7])),
}
...
def compare_periods(metric: str, period_a: str, period_b: str) -> dict:
    ...
    Args:
        metric: одна из "key_rate", "fx_USD", "fx_EUR", "fx_CNY", "cpi", "unemployment"
        period_a: "YYYY-MM" (для месячных метрик) или "YYYY-MM-DD" (для дневных)
    ...
    Returns:
        {"metric": ..., "a": {"date": ..., "value": ...},
         ...
         "source": "..."}
    """
    fn = METRIC_TO_FUNC.get(metric)
    if fn is None:
        return {"error": f"неизвестная метрика: {metric}. Допустимые: {list(METRIC_TO_FUNC.keys())}"}

    res_a = fn(period_a)
    res_b = fn(period_b)

    # извлечь числовое значение из результата инструмента
    value_keys = {
        "get_fx_rate": "rate",
        "get_key_rate": "rate",
        "get_inflation": "cpi_yoy",
        "get_unemployment": "unemployment",
    }

    def _extract_val(res: dict) -> tuple:
        if "error" in res:
            return None, None, res["error"]
        # определим, какой инструмент был вызван
        if "rate" in res:
            val_key = "rate"
        elif "cpi_yoy" in res:
            val_key = "cpi_yoy"
        elif "unemployment" in res:
            val_key = "unemployment"
        else:
            return None, None, f"не удалось извлечь значение из {res}"
        return res.get("date", ""), res[val_key], res.get("source", "")

    date_a, val_a, src_a = _extract_val(res_a)
    date_b, val_b, src_b = _extract_val(res_b)

    if val_a is None:
        return {"error": f"не удалось получить значение для period_a={period_a}: {src_a}"}
    if val_b is None:
        return {"error": f"не удалось получить значение для period_b={period_b}: {src_b}"}
